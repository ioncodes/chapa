...
#[test]
fn nested_set_get() {
    let c = Child::zeroed().with_high(2).with_low(1);
...
fn parent_with_child() {
    let c = Child::zeroed().with_high(3).with_low(2);
    let p = Parent::zeroed().with_child(c);
...
#[test]
fn parent_mixed() {
    let c = Child::zeroed().with_high(1).with_low(0);
    let p = Parent::zeroed()
        .with_child(c)
        .with_other(0xF)
        .with_tail(0xA);
