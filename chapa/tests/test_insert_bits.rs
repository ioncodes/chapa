...
fn lsb0_only_masked_src_bits_used() {
    ...
    let merged = insert_bits!(lsb0 u32; 0u32; 0xFFFF_FFFFu32; 8..=15);
    assert_eq!(merged, 0x0000_FF00);
}

#[test]
fn lsb0_single_bit() {
    let merged = insert_bits!(lsb0 u8; 0u8; 0xFFu8; 3);
    assert_eq!(merged, 0b0000_1000);
}

#[test]
fn lsb0_trailing_comma() {
    let a = insert_bits!(lsb0 u16; 0u16; 0xFFFFu16; 0..=3, 12..=15,);
    let b = insert_bits!(lsb0 u16; 0u16; 0xFFFFu16; 0..=3, 12..=15);
    assert_eq!(a, b);
    assert_eq!(a, 0xF00F);
}
...
fn msb0_single_and_range() {
    ...
    let merged = insert_bits!(msb0 u32; 0u32; 0xFFFF_FFFFu32; 0, 16..=31);
...
fn msb0_preserves_unselected_bits() {
    let dst: u32 = 0x1234_5678;
    let merged = insert_bits!(msb0 u32; dst; 0u32; 0, 16..=31);
...
fn explicit_form_is_const() {
    const MERGED: u32 = insert_bits!(lsb0 u32; 0x1234_5678u32; 0xFFu32; 0..=7);
...
fn round_trip_with_extract_bits() {
    ...
    let merged = insert_bits!(lsb0 u32; dst; extract_bits!(lsb0 u32; src; 8..=23); 8..=23);
